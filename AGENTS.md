...
- もう 1 つの例外は **Homebrew formula も mise backend も無い CLI tool** で、その場合だけ公式インストーラを呼ぶ専用の Tier 2 スクリプトを立てる。現状の対象は Notion CLI の `ntn`（`setup/notion.zsh`）。版は宣言側（`NTN_PINNED_VERSION`）で固定し、インストーラ既定の `latest`（導入した日で版が決まる）には倒さない。更新は dotfiles 側の明示変更で行う
