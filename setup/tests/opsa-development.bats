#!/usr/bin/env bats
# setup/tests/opsa-development.bats

REPO_ROOT="$(cd "$(dirname "${BATS_TEST_FILENAME}")/../.." && pwd)"
SCRIPT="${REPO_ROOT}/scripts/opsa-development.zsh"
...
#
# テンプレートに書く参照は架空の Vault / item 名だけを使う。公開リポジトリに実在の
# Vault 名・item 名・token 名を焼き込まないため。
setup() {
    STUB_BIN="${BATS_TEST_TMPDIR}/bin"
    STUB_OUT="${BATS_TEST_TMPDIR}/op-observed"
    ENV_FILE="${BATS_TEST_TMPDIR}/development.env"
    mkdir -p "${STUB_BIN}"
...
STUB
    ...
    export STUB_TOKEN="stub-development-service-account-token"
    export STUB_OUT
    export OPSA_DEVELOPMENT_ENV_FILE="${ENV_FILE}"

    write_env 'EXAMPLE_API_KEY=op://Example Vault/example-item/credential'
}
...
@test "run injects the Keychain token into the child process only" {
    run zsh "${SCRIPT}" run -- jev-dev --dry-run
    [ "${status}" -eq 0 ]

    run cat "${STUB_OUT}"
    [[ "${output}" == *"argv:run --env-file ${ENV_FILE} -- jev-dev --dry-run"* ]]
    [[ "${output}" == *"token:match"* ]]
}
...
@test "run never prints the token on stdout or stderr" {
    run zsh "${SCRIPT}" run -- jev-dev --dry-run
    [ "${status}" -eq 0 ]
    [[ "${output}" != *"stub-development-service-account-token"* ]]
}

@test "the token is not left in the calling shell's environment" {
    run zsh -c "zsh '${SCRIPT}' run -- jev-dev >/dev/null; printenv OP_SERVICE_ACCOUNT_TOKEN; echo rc=\$?"
    [[ "${output}" == *"rc=1"* ]]
    [[ "${output}" != *"stub-development-service-account-token"* ]]
}

# op が起動したコマンドとその子孫は token を継承し得る。標準経路の残余リスクとして
...
@test "the child's exit code is passed through unchanged" {
    STUB_OP_EXIT=7 run zsh "${SCRIPT}" run -- jev-dev
    [ "${status}" -eq 7 ]
}

@test "run rejects op as the child command" {
    run zsh "${SCRIPT}" run -- op read "op://Example Vault/example-item/credential"
    [ "${status}" -eq 1 ]
    [[ "${output}" == *"op の直接実行"* ]]
    [ ! -f "${STUB_OUT}" ]
}

# macOS の FS は既定で case-insensitive なので、大文字綴りも同じ実行体に解決される。
@test "run rejects op regardless of letter case" {
    for name in OP Op oP; do
        run zsh "${SCRIPT}" run -- "${name}" read "op://Example Vault/example-item/credential"
        [ "${status}" -eq 1 ]
        [[ "${output}" == *"op の直接実行"* ]]
        [ ! -f "${STUB_OUT}" ]
    done
...
@test "run rejects op given by an absolute path" {
    run zsh "${SCRIPT}" run -- /usr/local/bin/OP read "op://Example Vault/example-item/credential"
    [ "${status}" -eq 1 ]
    [[ "${output}" == *"op の直接実行"* ]]
    [ ! -f "${STUB_OUT}" ]
}
...
@test "a missing env-file fails closed" {
    run zsh "${SCRIPT}" run --env-file "${BATS_TEST_TMPDIR}/absent.env" -- jev-dev
    [ "${status}" -eq 1 ]
    [[ "${output}" == *"env-file が無い"* ]]
    [ ! -f "${STUB_OUT}" ]
}

# 参照だけのファイルでも「どの Vault のどの item か」は構成情報なので、
# owner 以外から読める配置のまま実行しない。
@test "a group/world readable env-file fails closed" {
    for mode in 644 640 604 660; do
        chmod "${mode}" "${ENV_FILE}"
        run zsh "${SCRIPT}" run -- jev-dev
        [ "${status}" -eq 1 ]
...
@test "a non-regular env-file fails closed" {
    local dir_env="${BATS_TEST_TMPDIR}/as-directory.env"
    mkdir -p "${dir_env}"
    chmod 700 "${dir_env}"
    run zsh "${SCRIPT}" run --env-file "${dir_env}" -- jev-dev
    [ "${status}" -eq 1 ]
    [ ! -f "${STUB_OUT}" ]
}

@test "an env-file holding a literal value is rejected" {
    write_env 'EXAMPLE_API_KEY=plaintext-secret'
    run zsh "${SCRIPT}" run -- jev-dev
    [ "${status}" -eq 1 ]
    [[ "${output}" == *"op:// 参照以外"* ]]
    [ ! -f "${STUB_OUT}" ]
...
@test "an unquoted reference with trailing garbage is rejected" {
    write_env 'EXAMPLE_API_KEY=op://Example Vault/example-item/credential よけいな文字列'
    run zsh "${SCRIPT}" run -- jev-dev
    [ "${status}" -eq 1 ]
    [[ "${output}" == *"op:// 参照以外"* ]]
    [ ! -f "${STUB_OUT}" ]
...
@test "a quoted reference with trailing garbage is rejected" {
    write_env 'EXAMPLE_API_KEY="op://Example Vault/example-item/credential" junk'
    run zsh "${SCRIPT}" run -- jev-dev
    [ "${status}" -eq 1 ]
    [[ "${output}" == *"op:// 参照以外"* ]]
    [ ! -f "${STUB_OUT}" ]
...
@test "a reference missing the field segment is rejected" {
    write_env 'EXAMPLE_API_KEY=op://Example Vault/example-item'
    run zsh "${SCRIPT}" run -- jev-dev
    [ "${status}" -eq 1 ]
    [[ "${output}" == *"op:// 参照以外"* ]]
    [ ! -f "${STUB_OUT}" ]
...
@test "comments and blank lines in the env-file are tolerated" {
    write_env '# development 用の参照だけ' '' 'EXAMPLE_API_KEY=op://Example Vault/example-item/credential'
    run zsh "${SCRIPT}" run -- jev-dev
    [ "${status}" -eq 0 ]
}
...
@test "a missing Keychain entry fails closed without running op" {
    ...
    run zsh "${SCRIPT}" run -- jev-dev
    [ "${status}" -eq 1 ]
    [[ "${output}" == *"OP_SERVICE_ACCOUNT_TOKEN_DEVELOPMENT"* ]]
    [ ! -f "${STUB_OUT}" ]
}

# Development 用の token は Infrastructure 用とは別の Keychain entry から取る。
@test "the Development wrapper does not read the infrastructure Keychain service" {
    run grep -c 'OP_SERVICE_ACCOUNT_TOKEN_INFRA' "${SCRIPT}"
    [ "${status}" -ne 0 ]
}

@test "--env-file overrides the default and is passed through to op" {
    local other="${BATS_TEST_TMPDIR}/other.env"
    printf 'EXAMPLE_API_KEY=op://Example Vault/example-item/credential\n' > "${other}"
    chmod 600 "${other}"
    run zsh "${SCRIPT}" run --env-file "${other}" -- jev-dev
    [ "${status}" -eq 0 ]
    run cat "${STUB_OUT}"
    [[ "${output}" == *"--env-file ${other}"* ]]
}

@test "OPSA_DEVELOPMENT_ENV_FILE selects the template when --env-file is absent" {
    local other="${BATS_TEST_TMPDIR}/from-env.env"
    printf 'EXAMPLE_API_KEY=op://Example Vault/example-item/credential\n' > "${other}"
    chmod 600 "${other}"
    OPSA_DEVELOPMENT_ENV_FILE="${other}" run zsh "${SCRIPT}" run -- jev-dev
    [ "${status}" -eq 0 ]
    run cat "${STUB_OUT}"
    [[ "${output}" == *"--env-file ${other}"* ]]
}

@test "the default template lives under a development-specific directory" {
    run grep -c '\.config/opsa-development/development\.env' "${SCRIPT}"
    [ "${status}" -eq 0 ]
}
