...

{
  home.packages = with pkgs; [
    ...
    # hermes-agent (~/.hermes) が要求する外部コマンド (ffmpeg: TTS 音声合成、
    # ripgrep: ファイル検索)。インストーラは不在を検出すると `brew install` に
    # フォールバックするが、default role の cleanup = "zap" が次の switch で宣言外
    # パッケージとして削除するため、放置すると削除と入れ直しが循環する。ここで
    # 宣言しておくとインストーラが検出して brew 経路に入らない。
    ffmpeg
    ripgrep
