...
- 状態を短く要約したら、そこで止まってユーザーの明示的な指示を待つ。次アクションの提案や AskUserQuestion を先回りで出さない。ユーザーはレビュー中か、自分の次手を持っていることが多い
- handoff に書かれた意図は spec / PR と突き合わせてから実装に入る。rename・削除・ENV 変更の「どちら向きか」を推測で決めない

## 後任セッションへの引き継ぎ

コンテキスト逼迫でセッションを世代交代させるときは herdr-succession skill の手順に従う。文書を書いて終わりにせず、後任の起動と、後任が状態を実測で把握したかの確認まで通す。
