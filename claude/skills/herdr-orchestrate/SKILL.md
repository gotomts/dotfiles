...
**グループの由来は 1 つではない。** よくあるのは次の 3 つで、どれも同じ prefix の仕組みに乗る:

- Linear project (「v2 メジャーアップデート」のような大きな取り組み)
- monorepo のサブディレクトリや領域 (`web/`, `backend/` など、issue tracker と対応しない区切り)
- ユーザーが頭の中で持っているだけの区切り (「今週の緊急対応」など)

したがって **Linear project から自動で決めようとしない**。project が付いていない issue も、そもそも Linear を使わないリポジトリも普通にある。
...
一度使った短縮キーは既存タブに残るので、2 回目以降は聞かれない。同じグループに別のキーを付けると分割が壊れるため、候補を作る前に必ず既存 prefix を見る。

グループが 1 つしか無いリポジトリでは prefix を付けない。分割の必要が無いところに区切りを持ち込むと、タブ名が長くなるだけで何も読みやすくならない。既存タブに prefix 付きが 1 つも無く、ユーザーも指定しなかったら、そのまま `ABC-123` で作る。
...
グループが 1 つしか無ければ見出しを出さずに平らに並べる。区切りが 1 つのときに見出しを付けても情報が増えない。
