...
7. **推奨スキル** (`parent.md` §2「推奨スキルの受け渡し」で、選定 dev-* の `skills:` 許可リストと issue の `### 推奨スキル` を交差させた結果。交差が空なら省略)

> **モデル格上げ:** HITL タグ付き、または親が設計重と判断したスライスは、`subagent_type` はそのままに `model="opus"` で起動する (`parent.md` §10)。作者格上げでも足りない設計判断は親直実装 (Phase 2-B 相当) に切り替える。
...
8. 期待される処理:
   ...
   - (auto-merge 有効化を指示された場合のみ) `gh pr merge --auto` で自動マージを予約
   - 結果を親に通知
9. **auto-merge の可否**: AI 自走マージ可なら「有効化せよ」、HITL/設計重 (親が Opus 格上げ・親直実装したもの含む) なら「有効化しない」を明示する (`parent.md` §5「選択的 auto-merge」「§10」参照)
