...
引数が不足している、パスが存在しない、もしくは `.claude/project.yml` が存在しない / `tracker.type` 未定義の場合は即座にエラーで停止する。**この時点でユーザー対話には戻らない**（呼び元が再投入する想定）。
...
`abort` は標準エラーにメッセージを出して停止する想定。失敗した場合、呼び元（`pick-next` / ユーザー直接実行）が再投入する。
...
- **重大な問題**（前提モジュールの不在、アーキテクチャ乖離、依存先 Issue 消失）→ ユーザーに報告して停止する。呼び元が spec / plan を直して呼び直す
...
呼び元 (`pick-next` / ユーザー / 後続 `feature-team` 起動) が利用できるよう、構造化された情報を返す:
...
### 次のステップ
ユーザーが `/feature-team <作成された番号>` で実装パイプラインを起動する (または `pick-next` の Step 7 に戻る)
```

## 失敗時の挙動

- 引数不正 / spec or plan 不在 → 即座にエラー停止（ユーザーが対話で直すのではなく、呼び元が再投入する想定）
- 認証エラー → 認証コマンドを案内して停止
- 重複検出 → ユーザーに報告して停止
...
- `pick-next` 連携 — pick-next が新規テーマ分岐で `Skill(create-issue)` を呼ぶ
- 単独実行 — spec / plan が手元にあるとき `/create-issue <spec> <plan>` で直接登録
- `feature-team` の前段 — `superpowers:brainstorming` + `superpowers:writing-plans` / `grill-me` / `grill-with-docs` などで作った spec / plan を issue 化してから `/feature-team <作成された番号>` を起動するパイプライン
