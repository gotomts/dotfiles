...
リポジトリ側に独自の issue-dev スキルが存在する場合、追加の手順を実行する。

**検出:**

リポジトリの worktree パス内で `.claude/skills/issue-dev/SKILL.md` の存在を確認する。

- 存在しない → 何もせずスキップ（出力なし）
- 存在する → SKILL.md を読み込み、記載された手順を実行

**コンテキスト:**

リポジトリ固有スキルの実行時、以下の情報が利用可能な状態で渡す:
