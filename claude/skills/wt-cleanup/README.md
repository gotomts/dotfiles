...
実行例は `SKILL.md` の前提・削除モードセクション参照。
...
| `pr-publisher` サブエージェント | コミット整形 + `git push` + `gh pr create` + `Skill(coderabbit-review)` までを担当。worktree 削除はしない（`pr-publisher.md` の「しないこと」に明記） |
...
- `@{u}` 未設定の場合 → 「upstream 未設定 (未 push)」として保護扱い（remote にコミットが存在しないため、削除すると差分を失う可能性がある）

### 3.3 force 引数によるガード解除 + 二段階確認（新規）

`/wt-cleanup force` を実行すると保護対象も削除候補に統合する。破壊的操作（uncommitted な変更や未 push commit を失う）が発生するため、確認は **2 段階** に分ける:

1. **1 段階目**: 保護対象を削除候補に統合する旨を保護理由付きで表示し、「保護対象を削除候補に加えてよいか」を `AskUserQuestion` で確認
2. **2 段階目**: 統合後の全削除対象 N 件について最終確認を `AskUserQuestion` で再度取得

通常モード（force なし）は最終確認の 1 段階のみ。
...
- **SKILL.md — 引数パーサ (case 文に追加)** に新規修飾子を追加する場合、本 README §4 の表も更新する（両方が同期している必要）
...
- **S6**: force — 保護対象を含むケースで `/wt-cleanup force` 実行 → 統合表示 → **1 段階目** 確認「保護対象 N 件を削除候補に統合します...続行しますか?」→ Y → **2 段階目** 確認「削除対象 N 件を削除します。続行しますか?」→ Y で削除実行
- **S7**: dry-run + force — `/wt-cleanup dry-run force` で保護解除済み削除候補が表示、確認プロンプトなし、実削除なし
- **S8**: 修飾子順序自由 — `with-pr force dry-run` と `dry-run with-pr force` で同等動作
- **S9**: 全件保護（force なし） — 全 worktree が保護対象になる状況で「クリーンアップ対象の worktree はありません。」表示後、即時 `exit 0`（dry-run であっても dry-run メッセージは出さない）
- **S9b**: upstream 未設定 — `git push` 前のブランチが「upstream 未設定 (未 push)」として保護カテゴリに表示されること
