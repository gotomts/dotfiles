...
モード指定は引数 `with-pr` で行う（口頭指示による自動判定は廃止）。明示的な引数指定がない場合は `merged` モードがデフォルト。
...
**このステップはあくまで一覧表示のみ。削除確認プロンプト（Y/n）はステップ 5 で行う。** dry-run 時はステップ 4 で終了するため、ユーザーが削除確認プロンプトを見ることはない。
...
まず削除確認プロンプトを表示し、ユーザーの承認を得る:

```
削除しますか？ (Y/n)
```

AskUserQuestion で確認し、`n` または空欄以外なら削除を実行する。`n` の場合はキャンセルして終了。

> **注**: `FORCE` フラグは現時点で未実装（Task B `KISSA-18` で追加予定）。force 引数を渡しても、現バージョンでは保護カテゴリは除外されたまま削除対象のみが削除される。
