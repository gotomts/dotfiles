...
- 推奨案には「この推奨が崩れる条件」を挙げ、その条件が対象リポジトリで成立していないことを確認してから提示する。確認の出所（ファイル:行）を示す。一般に良いとされる方法（小さい PR、分割、標準構成など）ほど成立条件を確かめずに使うため、定説を根拠にするときほど確認する
- 設計・レビュー・分析は分割確認せず、完全な形で一度に出力すること
- ユーザーがアプローチを修正・却下した場合は即座に受け入れて次に進む。却下されたアプローチを再提案したり、ユーザーが存在しないと言ったものを探し続けない（安全ルール・明示された要件・検証済みの技術的制約に反する却下は対象外。その場合は根拠を示して反論する）
...
- 非自明な WHY だけ。隠れた制約・workaround を入れた理由・読み手が驚く挙動といった、コードから復元できない情報に限る

書かないもの:

- WHAT（コードを読めば分かること。`// ユーザー ID を取得する` の類）
- 変更履歴（「〜を追加した」「旧実装では〜だった」）
- タスク ID 参照（`ABC-1234` 等）
- 設計メモ・受け入れ条件番号などの一過性文書（実装完了後にメンテナンスされない文書）への参照。文書が役目を終えると参照が陳腐化し、追跡不能な記述として残る
...
- その分野で確立している用語は、カタカナ・英語でも一般用語である。避けて和語のメタファーに置き換えると、かえって造語になる（AppBar → 「上部の帯」、BottomSheet → 「下から出す面」、Skeleton → 「骨組み」）
