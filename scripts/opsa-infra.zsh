#!/bin/zsh
# 1Password Service Account 経由でインフラ用の秘密を注入して実行する wrapper。
#
# 通常経路は `op run --env-file <template> -- <command>` の 1 本だけ。`op read` /
# `op item` / `op vault` のような値の直接取得・管理系はここを通さない（拒否する）。
#
# 使い方:
#   opsa-infra run [--env-file <path>] -- <command> [args...]
#   例: opsa-infra run -- terraform plan
#
# token は macOS Keychain の generic password（service 名 OP_SERVICE_ACCOUNT_TOKEN_INFRA）
# から取り、OP_SERVICE_ACCOUNT_TOKEN として `op run` プロセスの環境にだけ載せる。この shell
# では export せず、表示もファイル書き出しもしない。
#
# ただし **`op run` が起動するコマンドとその子孫は同じ環境を継承し得る**。terraform や
# provider から `op` を呼べば Service Account の権限がそのまま使える。これは標準経路の
# 残余リスクとして受け入れ、Vault scope（"Claude Code Infrastructure - Kissa Soft" だけ・
# read_items だけ・期限付き）で影響範囲を限定する。設計ドキュメント 4.4 を参照。
#
# これは AI に対する絶対境界ではない（同一 macOS user 上では別経路で同じ token に到達できる）。
# wrapper は事故防止のレールであって、境界は上記の Vault scope が作る。
# 設計と runbook: docs/superpowers/specs/2026-09-14-onepassword-claude-code-secret-boundary.md
#
# 終了コード:
...
# Keychain の service 名。runbook（設計ドキュメント 4 章）の保存側と同じ値を使う。
# Infrastructure Vault はプロジェクト別なので Service Account もプロジェクトごとに要る。
# 現状は Kissa Soft の 1 本だけを宣言しており、2 つ目が要るようになったら token と
# テンプレートを対で選ぶ入口（--project 等）を足す。今は作らない。
local keychain_service="OP_SERVICE_ACCOUNT_TOKEN_INFRA"

# op run に渡す env-file。秘密値ではなく `op://` 参照だけを書いた非追跡ファイルを指す。
local env_file="${OPSA_INFRA_ENV_FILE:-${HOME}/.config/opsa-infra/infra.env}"

opsa::die() {
  print -u2 "opsa-infra: ${1}"
  exit 1
}

opsa::usage() {
  print -u2 "使い方: opsa-infra run [--env-file <path>] -- <command> [args...]"
  print -u2 "  例:   opsa-infra run -- terraform plan"
...
# `opsa-infra run -- op read ...` のような、wrapper を踏み台にした値の直接取得を塞ぐ。
...
[[ -f "${env_file}" ]] || opsa::die "env-file が無い: ${env_file}"
...
while IFS= read -r line || [[ -n "${line}" ]]; do
  ...
  # vault/item/field の 3 段が必須。Vault 名 "Claude Code Infrastructure - Kissa Soft" の
  # ように空白を含む参照が正当なため、引用符で囲った形を標準とする（runbook もこの形）。
  #
